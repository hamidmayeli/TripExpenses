<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="cancel">
      <div class="bg-gray-800 text-gray-200 rounded-xl shadow-lg p-6 max-w-sm w-full mx-4 space-y-4">
        <p class="whitespace-pre-line">{{ message }}</p>
        <div class="flex justify-end gap-3">
          <button v-if="showCancel" @click="cancel"
            class="px-4 py-1.5 rounded bg-gray-600 hover:bg-gray-500 text-white">
            Cancel
          </button>
          <button @click="ok"
